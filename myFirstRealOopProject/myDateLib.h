...
public:
    ...
    clsDate(string date)
    {
        ...
        vDate = myStringLib::splitString(date, "/");
    ...
    clsDate(int dayFromTheBeginningOfTheYear, short year)
    {

        DaysFromTheBeginingOfTheYearToDate(dayFromTheBeginningOfTheYear, year);
    ...
    static vector<string> splitTime(string time)
    {

        return myStringLib::splitString(time, " ");
    ...
    static  bool checkIfLeapYear(short year) {
    ...
    enDays whatDayIsIt(clsDate date) {

        string arrOfDays[] = { "sunday","monday","tuesday","wednesday","thursday","friday","saturday" };
        short dayOrder = dayOfWeekOrder(date._year, date._month, date._day);
        return (enDays)dayOrder;
    ...
    short howManyDayFromTheBeginingOfTheYear(clsDate date) {
        short c = 0;
        for (short i = 1; i < date._month; i++)
        {
            c += dayInMonth(date._year, i);
        }
        c += date._day;
    ...
    short howManyDayFromTheBeginingOfTheYear() {

        howManyDayFromTheBeginingOfTheYear(*this);

    }

    void DaysFromTheBeginingOfTheYearToDate(int days, short year) {
        short month = 1;
        short day = 0;
        for (short i = 1;; i++)
        {
    ...
    void timeInYear(clsDate date) {

        cout << "number of days " << "in [" << date._year << "] is " << numberOfDays(date._year);
        cout << endl;
        cout << "number of hours " << "in [" << date._year << "] is " << numberOfHours(date._year, date._month);
        cout << endl;

        cout << "number of minutes " << "in [" << date._year << "] is " << numberOfMinutes(date._year, date._month);
        cout << endl;

        cout << "number of seconds " << "in [" << date._year << "] is " << numberOfSeconds(date._year, date._month);
    ...
    void printDayAndDate(clsDate date) {
        whatDayIsIt(date._year, date._month, date._day);
    ...
    void addDaysToDate(clsDate& date) {
        ...
        short timeRemainingInMonth = dayInMonth(date._year, date._month) - date._day;
        ...
        for (; i <= 12; i++)
        {
            ...
            if (daysToAdd <= dayInMonth(date._year, i)) {
                date._day = daysToAdd;
                date._month = i;
                dateToString(date);
                return;
            ...
            if (i == 12 && daysToAdd > dayInMonth(date._year, i)) {
                daysToAdd -= dayInMonth(date._year, i);
                date._year++;
                i = 0;
                continue;
            }
    ...
    bool  isDateOneBeforeDateTwo(clsDate date1, clsDate date2) {


        return date1._year > date2._year ? false : (date1._year == date2._year ? (date1._month > date2._month ? false : date1._month == date2._month ? (date1._day < date2._day) : true) : true);
    ...
    bool isDateOneEqualToDateTwo(clsDate date1, clsDate date2) {
    ...
    bool isDateOneEqualToDateTwo(clsDate date2) {

        isDateOneEqualToDateTwo(*this, date2);
    ...
    bool isDateOneAfterDateTwo(clsDate date1, clsDate date2) {
    ...
    bool isAfterDate(clsDate date2) {
        ...
        isDateOneAfterDateTwo(*this, date2);
    ...
    static  bool isValidDate(clsDate date) {
        ...
        if (date._day < 1 || date._day>31 || date._day > dayInMonth(date._year, date._month))
            return false;
